...
const dataPath = path.join(__dirname, '../data/toto-global-references.json');
...
if (!fs.existsSync(dataPath)) {
  console.error('Run: node scripts/scrape-toto-global.cjs first');
  process.exit(1);
}
...
const global = JSON.parse(fs.readFileSync(dataPath, 'utf8'));
...
console.log(
  `TOTO global import: +${added} new (${skipped} skipped, ${global.length} in source).`
);
